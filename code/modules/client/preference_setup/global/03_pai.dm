...
// End of Bastion of Endeavor Edit
	...
	*/
	...
	. += "Комментарии OOC: <a href='?src=\ref[src];option=ooc'>[candidate.comments ? TextPreview(candidate.comments, 40) : "Не установлены"]</a><br>"
	...
	if(href_list["option"])
		...
		switch(href_list["option"])
			...
			if("ooc")
				...
				*/
				t = tgui_input_text(user, "Введите свои комментарии ООС в качестве персонального ИИ:", "Комментарии ООС ПИИ", html_decode(candidate.comments), multiline = TRUE, prevent_enter = TRUE)
				// End of Bastion of Endeavor Translation
				if(!isnull(t) && CanUseTopic(user))
